...

{
	...
	var $temp_dir = './temp/images/branch/';
	var $temp_thumb_dir = './temp/images/branch/thumb/';
	var $upload_dir = './upload/images/branch/';
	var $upload_thumb_dir = './upload/images/branch/thumb/';
	...
	// function importNow($clinic){
	// 	$data['branch_name'] = $clinic['name'];
	// 	$data['branch_phone'] = $clinic['contact'];
	// 	$data['branch_address'] = $clinic['address'];
	// 	$data['branch_lat'] = $clinic['lat'];
	// 	$data['branch_lon'] = $clinic['lng'];
	// 	$data['branch_city'] = $clinic['branch_area'];
	// 	$data['status'] = 1;
	// 	$data['date_add'] = date('Y-m-d H:i:s');
	// 	$result = $this->db->insert('branch_item', $data);
	// }
	...
	function _editItem()
	{
		...
		if ($data) {
			...
			// delete image
			$this->db->where($where);
			$result = $this->db->update('branch_item', $data);
			if ($result) {
				if (!file_exists($this->upload_dir . $data['image_src'])) {
					if (!is_dir($this->upload_dir)) {
						mkdir($this->upload_dir, 0777, TRUE);
	...
	{
		$deleteid = $this->input->post('id_branch_item');
		if ($deleteid) {
			$this->load->model('core/dbtm_model', 'dbtm');
			$this->db->where('id_branch_item', $deleteid);
			$to_delete = $this->db->get('branch_item');
			...
			$result = $this->dbtm->deleteItem('id_branch_item', $deleteid, 'branch_item');
			if ($result) {
				return true;
			}
			else {
	...
	function _multipleActionItem()
	{
		...
		case 'active':
			foreach($multiple_id as $key => $item) {
				$data['whr_id_branch_item'] = $item;
				$data['clmn_status'] = 1;
				$this->load->model('core/dbtm_model', 'dbtm');
				$params['table'] = 'branch_item';
