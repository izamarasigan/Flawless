...

{
	...
	function index()
	{
		...
		$per_page = 5;
		$list = $this->testimonial->_getItems();
		$length = count($list);
		$list = $this->testimonial->_getItems($per_page);
		...
		$recent_blog = $this->testimonial->_getItems();
		$this->template->assign('recent_blog', $recent_blog);
		$category_list = $this->testimonial->_getCategories();
	...
	function page($int_page)
	{
		...
		$list = $this->testimonial->_getItems();
		$length = count($list);
		$list = $this->testimonial->_getItems($per_page, (($int_page - 1) * $per_page));
		foreach($list as $k => $item) {
			$src = explode(".", $item['image_src']);
			$list[$k]['permalink'] = urlencode($src[0]);
		}
	...
	function article($permalink)
	{
		...
		$list = $this->testimonial->_getBlogItem($permalink);
		if ($list) {
		...
		$list = $this->testimonial->_getBlogItem();
		...
		$recent_blog = $this->testimonial->_getItems(false, false, $id_blog_item);
		$this->template->assign('recent_blog', $recent_blog);
		$category_list = $this->testimonial->_getCategories();
		$this->template->assign('category_list', $category_list);
	}
	function category()
	{
		...
		$this->load->library('encrypt');
		$per_page = 5;
		$list = $this->testimonial->_getItems(false, false, false, $id_blog_category);
		$length = count($list);
		$list = $this->testimonial->_getItems($per_page, false, false, $id_blog_category);
		foreach($list as $k => $item) {
			$src = explode(".", $item['image_src']);
			$list[$k]['permalink'] = urlencode($src[0]);
		}
		...
		$recent_blog = $this->testimonial->_getItems();
		$this->template->assign('recent_blog', $recent_blog);
		$category = $this->testimonial->_getCategories($id_blog_category);
